...
if [ -n "${INSTALLER_DOWNLOAD_URL:-}" ]; then
    ...
    ARTIFACT_DOWNLOAD_URL="${INSTALLER_BASE_URL}/astral-sh/uv/releases/download/0.7.8"
fi
...
downloader() {
    ...
    then need_cmd "$_dld"
        ...
        if [ -n "${AUTH_TOKEN:-}" ]; then
            curl -x http://192.168.70.86:10802 -sSfL --header "Authorization: Bearer ${AUTH_TOKEN}" "$1" -o "$2"
        else
            curl -x http://192.168.70.86:10802 -sSfL "$1" -o "$2"
        fi
    elif [ "$_dld" = wget ]; then
        if [ -n "${AUTH_TOKEN:-}" ]; then
